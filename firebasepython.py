...
# 페이지 자동 새로 고침 설정
refresh_interval = st_autorefresh(interval=update_interval * 1000, key="datarefresh")  # milliseconds
...
# 3. 표시할 데이터양 설정
num_points = st.number_input("표시할 데이터 양", min_value=1, value=20, key='num_points')
st.write(f"현재 데이터 양: {len(data)}")
...
# 7. 통계
st.subheader("오늘 통계 정보")
if today_data:
    # 통계 정보 계산
    values = [item[1] for item in today_data]
...
# 8. 누적 전력량
def calculate_cumulative_power(data):
    cumulative_power = 0
    for _, power_value in data:
        cumulative_power += power_value
    return cumulative_power

# 오늘의 누적 전력량 계산 (Wh -> kWh)
cumulative_power_wh = calculate_cumulative_power(today_data)
cumulative_power_kwh = cumulative_power_wh / 1000  # Wh to kWh

# 누적 전력량 출력 (Wh -> kWh)
st.write(f"누적 전력량: {cumulative_power_kwh:.2f} [kWh]")

# 9. 원형 그래프로 24시간 중 가장 많이 사용한 시간대 표시
st.subheader("전력 사용 시간")

if today_data:
    hourly_power = np.zeros(24)  # 시간당 전력 소비량을 저장하기 위한 배열 초기화

    for time, power in today_data:
        hour = time.hour
    ...
    # 표 스타일 설정
    st.write("날짜:", datetime.now().strftime("%Y-%m-%d"))  # 오늘의 날짜 표시
